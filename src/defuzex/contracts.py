"""Immutable, JSON-compatible public contracts for the DefuzeX v4 API."""
...
from .errors import DefuzeError, ValidationError
...
class Case:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(item, DefuzeXInput) for item in self.inputs):
            raise ValidationError("Case inputs must contain DefuzeXInput values")
...
class HistoryItem:
    ...
    test_input: DefuzeXInput
    submission: Submission

    def __post_init__(self) -> None:
        if not isinstance(self.test_input, DefuzeXInput) or not isinstance(
...
class JudgeBatchResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.error is not None and not isinstance(self.error, DefuzeError):
            raise ValidationError("Batch Judgment error must be a DefuzeError")
