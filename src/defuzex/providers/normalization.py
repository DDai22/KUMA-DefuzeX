...
from ..contracts import Case, DefuzeXInput, TestReport
from ..errors import ProviderError, ValidationError
from ..privacy import PRIVATE_DATA_FIELDS, contains_private_data
from ..requirements import validate_schema, validate_structured_input
...
def _bounded_inputs(value: Any, max_inputs: int) -> list[Any]:
    if isinstance(value, (str, bytes, Mapping, DefuzeXInput)):
        return [value]
...
def _input_parts(
    ...
    if isinstance(value, DefuzeXInput):
        return (
    ...
    raise ProviderError("Each Input must be text, structured JSON, or DefuzeXInput")
