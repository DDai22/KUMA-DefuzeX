...
from .exceptions import DefuzeError
...
def cmd_whoami(args: argparse.Namespace) -> int:
    base_url = args.base_url or os.environ.get("DEFUZEX_BASE_URL", DEFAULT_BASE_URL)
    client = DefuzeClient(base_url=base_url, timeout=args.timeout)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="defuzex")
    subparsers = parser.add_subparsers(dest="command", required=True)
    whoami = subparsers.add_parser(
        "whoami", help="validate DEFUZEX_API_KEY and show entitlements"
    )
