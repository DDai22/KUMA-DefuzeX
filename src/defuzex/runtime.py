...
def resolve_runtime_mode(*, allow_local: bool) -> RuntimeMode:
    ...
    raise DockerRequiredError(
        "DefuzeX runs must start inside the same Docker container as the Agent; "
...
def default_lock_path() -> Path:
    ...
    if runtime_root:
        return Path(runtime_root).expanduser() / "defuzex" / "active-run.lock"
    if os.name == "posix":
        # Without XDG_RUNTIME_DIR, POSIX processes must resolve one fallback lock
        # to preserve the single-active-Run invariant.
        return Path("/tmp/defuzex-active-run.lock")  # nosec B108
    return Path(tempfile.gettempdir()) / "defuzex-active-run.lock"


def default_runtime_root(mode: RuntimeMode) -> Path:
    if mode == "docker":
        # Runs use random child names; cleanup validates each child's parent/name.
        return Path("/tmp/defuzex")  # nosec B108
    return Path(tempfile.gettempdir()) / "defuzex"
    ...
    def acquire(self, run_id: str) -> None:
        ...
        try:
            ...
            raise RunAlreadyActiveError(
                "Another DefuzeX Run is already active in this container"
            ) from exc
        try:
            metadata = json.dumps(
                {
...
def ensure_repo_runtime_directory(repo_path: Path) -> Path:
    """Create the excluded .defuzex directory and idempotent Git ignore rule."""

    root = repo_path.expanduser().resolve()
    if not root.is_dir():
        raise ConfigurationError("repo_path must be an existing directory")
    runtime_directory = root / ".defuzex"
    ...
    if ".defuzex/" in normalized_rules or "/.defuzex/" in normalized_rules:
    ...
    if updated and not updated.endswith(("\n", "\r")):
        updated += separator
    updated += f"/.defuzex/{separator}"
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".gitignore.defuzex-",
        suffix=".tmp",
        dir=root,
        text=True,
    )
