...
def credential_path(environ: Mapping[str, str] | None = None) -> Path:
    ...
    override = env.get("DEFUZEX_CONFIG_HOME")
    if override:
        return Path(override).expanduser() / "credentials.json"
    if os.name == "nt" and env.get("APPDATA"):
        return Path(env["APPDATA"]) / "DefuzeX" / "credentials.json"
    ...
    if configured_home:
        ...
        try:
            ...
            raise ConfigurationError(
                "Credential location unavailable; set DEFUZEX_API_KEY explicitly"
            ) from exc
    return base / "defuzex" / "credentials.json"


def validate_api_key(api_key: str) -> str:
    if not isinstance(api_key, str) or not api_key.startswith("dfx_"):
        raise ConfigurationError("DefuzeX API keys must begin with 'dfx_'")
    try:
        encoded = api_key.encode("ascii")
    except UnicodeEncodeError:
        raise ConfigurationError("DefuzeX API key format is invalid") from None
    if (
        len(encoded) <= 4
        or len(encoded) > _MAX_API_KEY_BYTES
        or any(byte < 0x21 or byte > 0x7E for byte in encoded)
    ):
        raise ConfigurationError("DefuzeX API key format is invalid")
    return api_key
...
def read_stored_api_key(*, path: Path | None = None) -> str | None:
    ...
    try:
        ...
        raise ConfigurationError(
            "The DefuzeX credential file is unreadable or invalid"
        ) from exc
    if not isinstance(payload, dict) or not isinstance(payload.get("api_key"), str):
        raise ConfigurationError("The DefuzeX credential file is invalid")
...
def resolve_api_key(
    ...
    *,
    ...
    env = os.environ if environ is None else environ
    candidate = (
        explicit or env.get("DEFUZEX_API_KEY") or read_stored_api_key(path=stored_path)
    )
    if candidate is None:
        if required:
            raise AuthenticationError(
                "Set DEFUZEX_API_KEY, pass api_key, or call defuzex.configure()."
            )
        return None
