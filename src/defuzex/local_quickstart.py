...
QUICKSTART_INPUT = "Reply with exactly: defuzex-ready"
QUICKSTART_EXPECTED_OUTPUT = "defuzex-ready"
...
def _write_artifact(*, output: str, score: int, reason: str) -> Path:
    artifact_root = Path(tempfile.mkdtemp(prefix="defuzex-local-result-")).resolve()
...
def run_local_quickstart(*, demonstrate_failure: bool = False) -> LocalQuickstartResult:
    ...
    with tempfile.TemporaryDirectory(prefix="defuzex-local-work-") as temporary:
